...
def listThemes(dataFB):
     ...
     return {
          "success": 1,
          "messages": "Theme list successfully retrieved.",
          "data": themes,
...
def findTheme(dataFB, themeName):
     ...
     return {
          "success": 1,
          "messages": "Theme found successfully.",
          "data": matched,
     }
...
def changeTheme(dataFB, threadID, themeName, initiatorID=None, timeout=_DEFAULT_TIMEOUT):
     ...
     return {
          "success": 1,
          "messages": "Theme change request sent successfully.",
          "data": {
...
* Required args:
     ...
     - threadID: group / thread ka ID jiska background badalna hai
     - themeName: theme name, partial theme name, theme id, ya "list"
     - initiatorID: action karne wale ka ID (optional, default current FacebookID)
...
* Example:
     ...
     dataFB = dataGetHome("<Facebook cookie>")
...
* Return result:
     ...
     - {"error": 1, "messages": "...", "details" | "payload" | "raw": ...} — fail hone par

* Notes:

     - listThemes GraphQL friendly_name="MWPThreadThemeQuery_AllThemesQuery" use karta hai.
     - changeTheme publish 4 MQTT LS tasks: ai_generated_theme, msgr_custom_thread_theme,
       thread_theme_writer, thread_theme.
     - Success ka matlab hai theme change request /ls_req par publish ho gayi; Messenger phir bhi reject kar sakta hai
       agar account ko us thread mein theme badalne ka adhikar nahi hai.
"""
