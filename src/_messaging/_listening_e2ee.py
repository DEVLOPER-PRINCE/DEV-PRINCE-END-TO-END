...
Facebook Messenger ke E2EE (Secret Conversations / Labyrinth) messages
ko decrypt karke sunna — Go binary `fbchat-bridge-e2ee` se stdin/stdout
(line-delimited JSON-RPC) ke zariye baat karta hai.

ctypes/dll version ke mukable fayde:
- Workspace mein `meta-messenger.js/` folder hona zaroori nahi.
- ctypes se shared library load nahi karni padti (zyada safe — bridge crash hone
  par Python crash nahi hoga).
- Bridge ek akele .exe ke roop mein distribute ho sakta hai.

Binary build karne ka tarika (ek baar):
    cd fbchat-v2/bridge-e2ee
    git clone https://github.com/mautrix/meta.git ./meta
    go mod tidy
    go build -ldflags="-s -w" -o ../build/fbchat-bridge-e2ee.exe .

Binary path override karne ke liye env: FBCHAT_E2EE_BIN=/path/to/binary

Pure Python kyun nahi?
----------------------
Messenger E2EE decrypt karne ke liye Signal Protocol chahiye (Curve25519,
Double Ratchet, Sender Keys, AES-GCM, HKDF, Noise XX) + Meta ka internal
protocol (Labyrinth / Lightspeed). Total ~100k LOC Go audited code hai,
Python mein koi equivalent lib nahi. Khud re-implement karna = high security
risk + Meta ke protocol change hone par maintain nahi ho sakta.
...
def _resolve_binary() -> Path:
    ...
    if not candidate.exists():
        raise FileNotFoundError(
            f"Bridge binary {candidate} par nahi mili.\n"
            f"Chalao: cd fbchat-v2/bridge-e2ee && go build -o ../build/{candidate.name} .\n"
            f"Ya FBCHAT_E2EE_BIN env var set karo."
        )
    return candidate
...
class BridgeError(RuntimeError):
    """Bridge `ok:false` ya transport error return karta hai."""


class _BridgeProcess:
    """RPC client for fbchat-bridge-e2ee.

    - Ek thread stdout padta hai, `id` ke basis par response caller ko distribute karta hai ya
      event ko `events` queue mein dalta hai.
    - `call(method, params)` response milne tak block karta hai.
    """
...
# ---------------------------------------------------------------------------
# Public listener — _listening.py ke saath API compatible
# ---------------------------------------------------------------------------

class listeningE2EEEvent:
    """Messages suno (regular + E2EE).

    `_listening.py` ke `listeningEvent` ke saath compatible:
        l = listeningE2EEEvent(dataFB)
        l.connect_mqtt()       # blocking, backward compatible name
    ...
    def _build_cookie_dict(self) -> dict[str, str]:
        ...
        if missing:
            raise ValueError(
                f"E2EE bridge ke liye zaroori cookie missing: {missing}. "
                f"Available cookies: {list(cks)}"
            )
    ...
    def connect_mqtt(self) -> None:
        """Bridge subprocess shuru karo + Messenger se connect karo (blocking poll loop)."""
        binary = (
    ...
    def send_message(self, thread_id: int, text: str,
        ...
        if self._bridge is None:
            raise RuntimeError("Connect nahi hua — pehle connect_mqtt() call karo.")
