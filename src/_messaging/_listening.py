...
"""
Sabse pehle, shukriya un sabhi fbchat-v2 users ka jo project mein contribute kiye.
Ab tum requests ki jagah wss (websocket) se messages receive kar sakte ho.
Maine inhe users ke support se remake kiya hai! Shukriya sabka.
...
class listeningEvent:
     ...
     def __init__(self, dataFB):
          self.bodyResults = {
               "body": None, # Message ka content
               "timestamp": 0, # Message bhejne ka time
               "userID": 0, # Message bhejne wala (sender)
               "messageID": None, # Message ka ID
               "replyToID": 0, # Jahan message bheja ya receive karna hai (thread/user)
               "type": None, # user/thread
               "attachments": { # Bheja hua attachment
                    "id": 0, # attachment ka id
     ...
     def connect_mqtt(self):
          ...
          def _messenger_queue_publish(client: mqtt.Client, userdata, flags, rc):
               # GraphQL sync_sequence_id sirf nayi queue banane ke liye use hota hai.
               # Jab syncToken mil jaaye, lastSeqID ko MQTT lastIssuedSeqId/firstDeltaSeqId ke saath follow karna chahiye.
               ...
               try:
                    ...
                    if "errorCode" in j:
                         ...
                         if is_queue_overflow:  # ERROR_QUEUE_OVERFLOW
                            ...
                            self.retry_count = 0
                            self.mqtt.disconnect()
                            time.sleep(10)  # Reconnect se pehle delay
                            self.connect_mqtt()  # Reconnect full (regenerate session)
                          
                         return  # Error processing band karo
          ...
          def on_disconnect(client, userdata, rc):
                 ...
                 if rc != 0:  # Normal disconnect nahi hai
                      print("Unexpected disconnect - reconnecting in 10s...")
                      time.sleep(10)
                      self.connect_mqtt()  # Khud se reconnect
