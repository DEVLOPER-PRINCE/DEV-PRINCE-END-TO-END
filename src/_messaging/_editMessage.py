...
def _publish_ls_requests(dataFB, contexts, timeout=_DEFAULT_TIMEOUT):
     ...
     try:
          ...
          return {
               "success": 1,
               "messages": "LS task published successfully.",
               "payload": {
...
def editMessage(dataFB, messageID, newText, timeout=_DEFAULT_TIMEOUT):
     ...
     return {
          "success": 1,
          "messages": "Message edit request sent successfully.",
          "data": {
...
* Zaroori arguments (args):
     ...
     - messageID: edit karne wale message ka ID (usually sirf apne bheje messages edit ho sakte hain)
     - newText: naaya content
...
* Example:
     ...
     print(_editMessage.editMessage(dataFB, "mid.$...", "Edited content"))

* Return result:

     - {"success": 1, "messages": "...", "data": {...}} — LS task publish hone par
     - {"error": 1, "messages": "...", "payload": {...}} — MQTT error / data missing par

* Notes:

     - Yeh endpoint MQTT LS task queue_name="edit_message" use karta hai.
     - Facebook direct GraphQL response nahi deta; success ka matlab hai task /ls_req par publish ho gaya.
"""
