...
def _parse_graphqlbatch_response(text):
    """Facebook GraphQL batch mein kai JSON objects connected hote hain; split se nahi kaatnaa chahiye."""
    text = (text or "").strip()
    ...
    raise ValueError("GraphQL batch response mein o0 object nahi mila.")
...
def func(dataFB): # INBOX messages ke components ka data lo
    ...
    if last_seq_id is None:
        raise ValueError(f"sync_sequence_id valid nahi hai: {message_threads.get('sync_sequence_id')}")
    try:
        threadIDList = []
        threadNameList = []
...
def features(dataGet, threadID, commandUse):
    ...
    try:
        ...
        try:
            ...
            return "ThreadList data process nahi ho saka."
    ...
    if (dataThread != None):
        match commandUse:
            case "getAdmin":  # Thread Admin ka ID lo
            ...
            case "threadInfomation": # Thread ki info lo
                ...
                exportData = {
                        ...
                        "approvalMode": "On" if (dataThread["approval_mode"] != 0) else "Off",
                        "joinableMode": "On" if (dataThread["joinable_mode"]["mode"] != "0") else "Off",
                        "urlJoinableThread": dataThread["joinable_mode"]["link"]
                }
            case "exportMemberListToJson": # Thread member data ko JSON mein export karo
        ...
        return "ThreadList data nahi mila, error aa gaya T___T"