...
def func(dataFB, threadID, newNameThread): # Group ka naam badlo
    ...
    if (sendRequests.get("error")):
        match sendRequests.get("error"):
            case 1545012:
                return formatResults("error", "Aap group member nahi hain isliye group name change nahi kar sakte.")
            case 1545003:
                return formatResults("error", "Jo group exist nahi karta uska naam change nahi ho sakta.")
    else:
        return formatResults("success", "Group name successfully changed.")