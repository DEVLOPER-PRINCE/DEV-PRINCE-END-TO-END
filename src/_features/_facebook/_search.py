...
def func(dataFB, keywordSearch): # Facebook par search karo
    
    # Data liya aur likha gaya: 01:42 Thursday, 06/07/2023. Author: Prince Malhotra
    """Args:
    ...
    try:
        ...
        for dataResults in getDataResultSearch:
            ...
            listResultSearch.append("🔮User Name: " + dataResults["view_model"]["profile"]["name"] + "\n⚗️User ID: " + dataResults["view_model"]["profile"]["id"] + "\n🏷️Profile Link: " + dataResults["view_model"]["profile"]["url"] + "\n≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈")
        return {
            "success": 1,
            "searchResults": "≈ ≈ ≈ Facebook Search ≈ ≈ ≈\n\n" + "\n".join(listResultSearch) + "\n🔎Search keyword: " + str(keywordSearch) + "\n📊Result count: 5",
